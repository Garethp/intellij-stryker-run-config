package com.blackfireweb.stryker.run.ui

import com.blackfireweb.stryker.run.StrykerRunConfig
...
class ConfigurableEditorPanel(private val myProject: Project) : SettingsEditor<StrykerRunConfig>(), PanelWithAnchor {
    ...
    private val kindButtons: Array<JRadioButton> = Array(StrykerRunConfig.TestKind.values().size) { JRadioButton(StrykerRunConfig.TestKind.values()[it].myName) }
    ...
    private val myRadioButtonMap: MutableMap<StrykerRunConfig.TestKind, JRadioButton> = EnumMap(StrykerRunConfig.TestKind::class.java)

    private val myTestKindViewMap: MutableMap<StrykerRunConfig.TestKind, TestKindView> = EnumMap(StrykerRunConfig.TestKind::class.java)
    ...
    private fun createTestKindRadioButtonPanel(): JPanel {
        ...
        StrykerRunConfig.TestKind.values().forEachIndexed { index, testKind ->
    ...
    private fun setTestKind(testKind: StrykerRunConfig.TestKind) {
    ...
    private fun getTestKindView(testKind: StrykerRunConfig.TestKind): TestKindView {
    ...
    public override fun applyEditorTo(configuration: StrykerRunConfig) {
        ...
        data.kind = getTestKind() ?: StrykerRunConfig.TestKind.SPEC
        val view = this.getTestKindView(data.kind)
        view.applyTo(data)
    }

    public override fun resetEditorFrom(configuration: StrykerRunConfig) {
    ...
    private fun createUIComponents() {
        ...
        myCommonParams = ProgramParametersPanel()
        (myCommonParams as ProgramParametersPanel).workingDir.label.text = "Project base:"
    }
...
class ProgramParametersPanel : CommonProgramParametersPanel(true) {
